...
### 4) Resolve dependencies across manifests (replaces `pkgdepend resolve`)

- API: Resolver::resolve(&mut manifests)
- In pkgdev: call resolver on the set of package manifests.
...
- run_resolve_dependencies: Use Resolver::resolve(&mut [Manifest]) across the in-memory set of manifests. If DependencyGenerator::generate_with_repo has already resolved repository deps, this step will be a fast no-op; otherwise, it ensures cross-manifest mapping is finalized.
