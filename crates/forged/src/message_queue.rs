        ...
        Event::Create(envelope) => {
            ...
            match envelope.object {
                ...
                ActivityObject::ChangeRequest(change_request) => {
                    let db_cr = db.change_request().create(
                        vec![
                    ...
                    debug!("created change request with id: {} for reference: {}", db_cr.id, change_request.external_ref.to_string());
                    Ok(())
                }
            ...
            match envelope.object {
                ActivityObject::ChangeRequest(change_request) => {
                    ...
                    for component_change in change_request.changes {
                        ...
                        let mut component_where_args = vec![
                            prisma::component::name::equals(component_change.recipe.name.clone()),
                        ];
                                ...
                                match item.name.as_str() {
                                    "anitya-id" => {
                                        component_where_args.push(
                                            prisma::component::anitya_id::equals(Some(item.value.clone())),
                                        );
                                    }
                                    "repology-id" => {
                                        component_where_args.push(
                                            prisma::component::repology_id::equals(Some(item.value.clone())),
                                        );
                                    }
                                    _ => {}
                                }
                        ...
                        let component = db.component().find_first(vec![
                            prisma::component::WhereParam::Or(component_where_args)
                        ]).exec().await?;
                ...
                ActivityObject::Gate(gate) => {
                    db.gate().update(
                        prisma::gate::UniqueWhereParam::IdEquals(gate.id),
                        vec![
                            ...
                            prisma::gate::SetParam::ConnectPublisher(prisma::publisher::UniqueWhereParam::NameEquals(gate.publisher)),
                            prisma::gate::SetParam::SetTransforms(serde_json::to_value(&gate.default_transforms)?),
                        ]
                    ).exec().await?;
                    Ok(())
                }
