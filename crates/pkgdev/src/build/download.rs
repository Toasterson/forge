use crate::build::path::add_extension;
        ...
        for src in &section.sources {
            match src {
                component::SourceNode::Archive(archive) => {
                    ...
                    let local_file = wks.get_file_path(&url)?;
                    ...
                    if !archive_path.exists() {
                        ...
                        let mut easy =
                            Easy2::new(wks.open_local_file(&url, HasherKind::Sha512)?);
                ...
                component::SourceNode::Git(git) => {
                    ...
                    let archive_path = add_extension(
                        Settings::get_or_create_archives_dir().into_diagnostic()?.join(&git_prefix),
                        "tar.gz",
                    );
...
fn git_clone_get(wks: &Workspace, git: &component::GitSource) -> Result<()> {
...
fn make_git_archive_with_tar(wks: &Workspace, git: &component::GitSource) -> Result<()> {
    let repo_prefix = git.get_repo_prefix();
    ...
    let archive_name_arg = add_extension(
        Settings::get_or_create_archives_dir().into_diagnostic()?.join(&repo_prefix),
        "tar.gz",
    )
        .to_string_lossy()
        .to_string();
...
fn make_git_archive(wks: &Workspace, git: &component::GitSource) -> Result<()> {
    let repo_prefix = git.get_repo_prefix();
...
fn git_archive_get(wks: &Workspace, git: &component::GitSource) -> Result<()> {
    let mut git_cmd = Command::new("git");
    let repo_prefix = git.get_repo_prefix();
